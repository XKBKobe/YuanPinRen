...
class BindBankCard extends Component {
    ...
    confirmIdentiryInfo() {
        ...
        if (!positiveImgUrl || positiveImgUrl.length < 1 || !negativeImgUrl || negativeImgUrl.length < 1) {alert("请选择身份证照片！");return;}
        if (!idCard || idCard.length != 18) {alert("请输入正确的身份证号！");return;}
